    ...
    return (
      <div className={cn("grid gap-3 sm:gap-4", colClass)}>
        {Array.from({ length: cols }).map((_, i) => (
          <div key={`stat-skeleton-${i}`} className="ui-surface p-4 sm:p-5">
      ...
      {stats.map((stat) => {
        ...
        const content = (
          <StatElement
            ...
            className={cn(
              "ui-surface group relative min-w-0 overflow-hidden p-4 text-left transition-all sm:p-5",
              stat.onClick &&
          ...
          >
            <div className="mb-2 flex items-center justify-between gap-2">
              <span className="ui-label min-w-0 truncate">{stat.label}</span>
              {stat.icon && (
                <stat.icon
                  className={cn(
                    "size-4 opacity-70 transition-colors",
                    stat.color ?? "text-muted-foreground",
                    stat.onClick && "group-hover:text-primary",
                  )}
                />
              )}
            </div>
            <div className="break-words text-xl font-bold text-foreground sm:text-2xl">
              {stat.value}
            </div>
            {stat.onClick && (
