...
export function StockByLocationList({
  ...
  const totalAvailable = stocks.reduce((sum, item) => sum + Number(item.qtyAvailable || 0), 0);
            ...
            >
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${getAvailabilityClass(state)}`}
                  >
                    {state}
                  </span>
                </div>
...
export function ProductSectionTitle({ icon, title }: { icon: ReactNode; title: string }) {
  return (
