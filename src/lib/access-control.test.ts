...
describe("access control", () => {
  it("routes warehouse staff to picking and keeps admin settings restricted", () => {
    ...
    expect(canAccessPath("/purchase-orders", ["WAREHOUSE_STAFF"])).toBe(true);
    expect(canAccessPath("/purchase-orders/new", ["WAREHOUSE_STAFF"])).toBe(
      false,
    );
    expect(canAccessPath("/purchase-orders/new", ["WAREHOUSE_MANAGER"])).toBe(
      true,
    );
  ...
  it("keeps picking and security navigation aligned with role access", () => {
    const staffLabels = filterSidebarSections(SIDEBAR_SECTIONS, [
      "WAREHOUSE_STAFF",
    ]).flatMap((section) => section.items.map((item) => item.label));
    const adminLabels = filterSidebarSections(SIDEBAR_SECTIONS, [
      "ADMIN",
    ]).flatMap((section) => section.items.map((item) => item.label));
