...
function statusClass(status: CycleCountStatus) {
  ...
  if (status === "REVIEW" || status === "PENDING_REVIEW" || status === "RECOUNT_REQUIRED") return "border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-300";
...
function lineStats(count: CycleCount) {
  ...
  const counted = count.countedLines ?? lines.filter((line) => line.status === "COUNTED" || line.status === "VARIANCE" || line.status === "APPROVED" || line.status === "ADJUSTED").length;
  const total = count.totalLines ?? lines.length;
  const variances = count.discrepancyLines ?? lines.filter((line) => Number(line.discrepancy ?? line.varianceQty ?? 0) !== 0 || line.status === "VARIANCE" || line.status === "ADJUSTED").length;
  if (!total) return { label: "Chưa sinh dòng kiểm", variances: 0 };
  return { label: `${counted}/${total} đã kiểm`, variances };
}
...
export default function CycleCountsPage() {
  ...
  const canCreate = useHasPermissions(ADMIN_MANAGER_ROLES);
  const hasReportViewerRole = useHasPermissions("REPORT_VIEWER");
  const hasOperationRole = useHasPermissions(["ADMIN", "WAREHOUSE_MANAGER", "WAREHOUSE_STAFF"]);
  ...
  const { data, isLoading, isFetching, error, refetch } = useGetCycleCountsQuery({
    ...
    size: pageSize,
    keyword,
    status: isReportViewerOnly ? "APPROVED" : status,
    warehouseId,
  });
  ...
  const reviewCount = rows.filter((row) => row.status === "REVIEW" || row.status === "PENDING_REVIEW").length;
          ...
          right={
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Select
                ...
                disabled={isReportViewerOnly}
              >
                <SelectTrigger className="h-10 w-44 rounded-xl border-zinc-200 dark:border-zinc-700">
                  <span className={cn("truncate text-sm", !selectedStatus && "text-muted-foreground")}>
                    {selectedStatus ? STATUS_LABEL[selectedStatus] : "Tất cả trạng thái"}
                  </span>
                </SelectTrigger>
                <SelectContent className="rounded-xl">
                  <SelectItem value="all">Tất cả trạng thái</SelectItem>
                  {(["PENDING", "IN_PROGRESS", "PENDING_REVIEW", "APPROVED", "RECOUNT_REQUIRED", "CANCELLED"] as CycleCountStatus[]).map((value) => (
                    ...
                    <TableRow key={row.id} className="hover:bg-muted/50">
                      ...
                      <TableCell className="px-4 py-3 text-sm text-foreground">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          Giao cho: {row.assignedTo ? (staffNameById.get(row.assignedTo) ?? row.assignedTo) : "Chưa phân công"}
                        </div>
                      </TableCell>
                      <TableCell className="px-4 py-3">
